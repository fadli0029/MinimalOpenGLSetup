...
App::App(int width, int height, const std::string& title, const char* vertexShader, const char* fragmentShader) {
    ...
    physicsHandler_ = new BulletPhysics();
}

App::~App() {
    delete fpsInputHandler_;
    delete arcballInputHandler_;
    delete physicsHandler_;
}
...
void App::Update() {
    ...
    physicsHandler_->StepSimulation(deltaTime_, 10);
}
...
void App::Render() {
    ...
    float matrix[16];
    physicsHandler_->GetWorldTransform(matrix);
